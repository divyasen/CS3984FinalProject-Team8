...
public class PhotoFileManager implements Serializable {
    ...
    The instance variable 'userPhotoFacade' is annotated with the @EJB annotation.
    The @EJB annotation directs the EJB Container (of the GlassFish AS) to inject (store) the object reference 
    of the ListingFacade object, after it is instantiated at runtime, into the instance variable 'userPhotoFacade'.
     */
    @EJB
    ...
    public FacesMessage storeListingPhotoFile(UploadedFile file, Listing listing) {
        try {
            // This sets the necessary flag to ensure the messages are preserved.
            FacesContext.getCurrentInstance().getExternalContext().getFlash().setKeepMessages(true);
            ...
            List<Listing> allListing = getListingFacade().findListingsByUserID(user.getId());
            ...
            database primary key is user.getId()
             */
            List<ListingPhoto> photoList = getListingPhotoFacade().findPhotosByListingID(user.getId());

            // Construct a new Photo object with file extension and user's object reference
            ListingPhoto newPhoto = new ListingPhoto(fileExtension, listing);

            // Create a record for the new Photo object in the database
            getListingPhotoFacade().create(newPhoto);

            // Obtain the object reference of the first Photo object of the
            // user whose primary key is user.getId()
            ListingPhoto photo = getListingPhotoFacade().findPhotosByListingID(user.getId()).get(0);
